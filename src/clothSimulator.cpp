...
// NOTE: Commenting out cloth code
//void GalaxySimulator::loadCloth(Cloth *cloth) { this->cloth = cloth; }

//void GalaxySimulator::loadClothParameters(ClothParameters *cp) { this->cp = cp; }

void GalaxySimulator::loadCollisionObjects(vector<CollisionObject *> *objects) { this->collision_objects = objects; }
...
void GalaxySimulator::init() {
    ...
    // NOTE: Commenting out cloth code; could be used to determine camera pos for solar system
...
void GalaxySimulator::drawContents() {
    ...
    if (!is_paused) {
        ...
        for (int i = 0; i < simulation_steps; i++) {
            // TODO: Replace with code simulating every object from collision_objects
...
bool GalaxySimulator::keyCallbackEvent(int key, int scancode, int action,
    ...
    if (action == GLFW_PRESS) {
        switch (key) {
            ...
            case 'r':
            case 'R':
                // TODO: Commenting out cloth code; could be used to reset solar system simulation
//                cloth->reset();
                break;
            case ' ':
                resetCamera();
